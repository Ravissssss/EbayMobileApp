...
public class HomePage extends MobileActions {
	...
	public void homePage(MobileDriver driver, HashMap<String, String> data) throws Exception
	{
		try 
		{
			...
			Assert.assertEquals(tvPrice.getText(),data.get("Tv Price"),"Tv Price" );		
			Screenshots.takeScreenshot(driver, screenshotFilePath + "SelectProduct1.png");

			Log.info("Clicking on 65 inches TV");
			clickElement(driver, verificationErrors, tvPrice);

			Log.info("Verifying the Rotate Screen");	
			driver.switchTo();
			...
			Log.info("Verifying FirstName label");
			verifyElementPresent(driver, verificationErrors, lbl_FirstName);

			Log.info("Verifying LastName label");
			Assert.assertEquals(lbl_LastName.getText(),data.get("LabelLastName"),"LastName label" );
			...
			Assert.assertEquals(lbl_Password.getText(),data.get("LabelPassword"),"Password label" );
