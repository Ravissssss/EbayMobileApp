...
public class CustomReportTest implements IReporter {

	private static final Logger LOG = Logger.getLogger(CustomReport.class);
	private static String timeZone = "GMT+5:30";
	private static SimpleDateFormat sdfdate = new SimpleDateFormat("dd-MMM-yyyy HH:mm:ss");
	private static SimpleDateFormat sdftime = new SimpleDateFormat("HH:mm:ss");
	private static String outFilename = "Ebay_Custom_Report.html";
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites,
		...
		platform = xmlSuites.get(0).getTests().get(0).getParameter("MobileType");
		url = xmlSuites.get(0).getTests().get(0).getParameter("URL");
	...
	protected void writeHead() {
		writer.print("<head>");
		writer.print("<title>Ebay Mobile Test Execution Report</title>");
		writeStylesheet();
		writer.print("</head>");
	}
	...
	protected void writeBody() {
		writer.print("<body>");
		writeReportTitle( "Ebay Mobile Test Execution Report" );
		writer.print("<b>Ebay URL: "+ url+"</b>");
		writeSuiteSummary();
	...
	protected void writeReportTitle( String title ) {
		writer.print( "<center><h2>" + title + "&emsp;" + getDateAsString() + "</h2></center>" );
	}
	...
	protected void writeSuiteSummary() {
		...
		writer.print("<table id=\"top_page\">");
		writer.print("<tr>");
		...
		writer.print("<th>Time (HH:MM:SS:mmm)</th>");
		...
		for ( SuiteResult suiteResult : suiteResults ) {
			...
			writer.print("<tr><center><b>"+ Utils.escapeHtml(suiteResult.getSuiteName())+"</b></center></>" );
			//writer.print("<tr><th colspan=\"4\">");
			writer.print("<tr colspan=\"5\">");
		...
		if ( testIndex >= 1 ) {
			writer.print("<tr>");
			writer.print("<th>"+platform+"</th>");
			writer.print("<th>"+(totalPassedTests+totalFailedTests)+"</th>");
			...
			writeTableHeader( millisecondsToHour_Minutes(totalDuration), "<center>num</center>");
			//  writer.print("<th colspan=\"0\"></th>");
			writer.print("</tr>");
		}
	...
	protected void writeScenarioSummary() {
		...
		writer.print("<thead>");
		writer.print("<tr>");
		writer.print("<th>Workspace</th>");
		writer.print("<th>App</th>");
		writer.print("<th>TestScenario</th>");
		writer.print("<th>Start</th>");
		writer.print("<th>End</th>");
		writer.print("<th>Time Taken (MM:SS)</th>");
		writer.print("</tr>");
		writer.print("</thead>");
		...
		for (SuiteResult suiteResult : suiteResults ) {
			...
			System.out.println("Testing1---"+suiteResult.getSuiteName());
			//writer.print(Utils.escapeHtml(suiteResult.getSuiteName() ) );
			writer.print("</th></tr></tbody>");*/

			writer.print("<tbody><tr colspan=\"6\">");
			//System.out.println("Testing1---"+suiteResult.getSuiteName());
			//writer.print(Utils.escapeHtml(suiteResult.getSuiteName() ) );
			writer.print("</tr></tbody>");
			...
			for ( TestResult testResult : suiteResult.getTestResults() ) {
				...
				//System.out.println("Browser Name : " + testResult.getParamName());
				//System.out.println("Testing2---"+testResult.getTestName());
	...
	private int writeScenarioSummary(String description, List<ClassResult> classResults,
		...
		if (!classResults.isEmpty()) {
			...
			//System.out.println("Testing3---"+description);
			writer.print("</th></tr>");
			...
			for ( ClassResult classResult : classResults ) {
				...
				for (MethodResult methodResult : classResult.getMethodResults() ) {
					...
					ArrayList<ITestResult> results = methodResult.getResults();
					...
					for (ITestResult iTestResult : results) {
						...
						//System.out.println("Method Result Iterator : -- "+iTestResult.getName());
						parameters = iTestResult.getParameters();
						//System.out.println("Parameter for this method: " +parameters[0]);
						...
						//	System.out.println("Testing4---"+methodName);
						...
						//System.out.println("Testing6---"+scenarioIndex);


						if(cssClassPrefix.equalsIgnoreCase("failed"))
						{

							buffer.append("<td><a href=\"#m").append(scenarioIndex)
							.append("\">").append( methodName + "</a></td>" )
							.append( "<td rowspan=\"1\">" + param + "</td>" )
							.append( "<td rowspan=\"")
							.append(resultsCount).append("\">").append( parseUnixTimeToTimeOfDay(start) )
							.append( "</td>").append("<td rowspan=\"" )
							.append(resultsCount).append("\">").append( parseUnixTimeToTimeOfDay(end) )
							.append( "</td>").append("<td rowspan=\"" )
							.append(resultsCount).append("\">")
							.append( millisecondsToMinutes_Seconds( duration )  ).append("</td></tr>");
							scenarioIndex++;

						}
						else
						{
							buffer.append("<td><a id=\"#m").append(scenarioIndex)
							.append("\">").append( methodName + "</a></td>" )
							.append( "<td rowspan=\"1\">" + param + "</td>" )
							.append( "<td rowspan=\"")
							.append(resultsCount).append("\">").append( parseUnixTimeToTimeOfDay(start) )
	...
	protected void writeScenarioDetails() {
		...
		for ( SuiteResult suiteResult : suiteResults ) {
			for ( TestResult testResult : suiteResult.getTestResults() ) {
				...
				int p=testResult.getFailedTestCount();
	...
	private void writeScenario(int scenarioIndex, String label,
		...
		if(s!=null)
		{
			if(s.contains("Build info:"))
			{
				error_Message = (s.substring(s.indexOf("selenium.")+9, s.indexOf("Build")-1));
			}

			else
			{	
				error_Message = s.replace("java.lang.", "");
			}
		}
		else
		{
		...
		writer.print("</table>");
		writer.print("<p class=\"totop\"><a href=\"#top_page\">back to summary</a></p>");
	}
	...
	protected static class SuiteResult {
		...
		public SuiteResult(ISuite suite) {
			...
			parameterName = suite.getParameter("Browser");
			urlName = suite.getParameter("URL");
	...
	protected static class TestResult {
		...
		protected List<ClassResult> groupResults(Set<ITestResult> results) {
			...
			if (!results.isEmpty()) {
				...
				ArrayList<ITestResult> resultsPerMethod = new ArrayList();
	...
	protected static class ClassResult {
		...
		public ClassResult(String className, List<MethodResult> methodResults) {
			String temp = className.substring(className.lastIndexOf(".")+1,className.length());
	...
	protected static class MethodResult {
		private final ArrayList<ITestResult> results;

		/**
		 * @param results the non-null, non-empty result list
		 */
		public MethodResult(ArrayList<ITestResult> results) {
		 ...
		 */
		public ArrayList<ITestResult> getResults() {
			return results;
		}
	}
	...
	protected String millisecondsToMinutes_Seconds( long ms ) {
		long mm = ms/ (60*1000);
		ms = ms - mm*(60*1000);
		long ss = 	ms / 1000;
	...
	protected String millisecondsToHour_Minutes( long ms ) {
